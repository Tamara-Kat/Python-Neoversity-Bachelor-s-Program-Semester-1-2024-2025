...
def change_permissions(fs: Dict, path: str, mode: int, *permissions: str) -> int:
    ...
    def find_and_change(current: Dict, path_parts: List[str]) -> bool:
        ...
        if len(path_parts) == 1:  # Якщо це остання частина шляху
            ...
            if 'contents' in target:
                for item in target['contents'].values():
                    item['access'] = modify_permissions(item['access'], set(permissions))
                    if 'contents' in item:
                        for subitem in item['contents'].values():
                            subitem['access'] = modify_permissions(subitem['access'], set(permissions))
            return True
...
print(file_system["root"]["contents"]["folder1"]["contents"]["subfolder1"]["contents"]["file1.txt"]["access"])
...
r = change_owner(file_system, "root/folder1", "new_admin")  # Статуси 0, 1, 2 відповідно для успішного, некоректного шляху та некоректної файлової системи
print(r)
